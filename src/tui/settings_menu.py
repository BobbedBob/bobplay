...
def run_settings_menu():
    ...
    while True:
        ...
        console.print()
        choice = Prompt.ask(f"  [bold {BLUE_ICE}]{ICON_PLAY} Select an option[/]", choices=["1", "2", "3", "0"],
                            default="0")
        ...
        if choice == "0":
            ...
            console.print(
                f"  [{GRAY_DARK}](To remove the status completely, just type the word 'none' or leave empty)[/]")
            ...
            new_val = IntPrompt.ask(f"  [bold {BLUE_LIGHT}]Enter new status type[/]", choices=["0", "2", "3", "5"],
                                    default=Config.get('bot_status_type'))
            Config.set("bot_status_type", new_val)

        elif choice == "3":
            console.print(f"\n  [{STEEL}]Current URL:[/] {Config.get('controller_idle_image')}")
            new_val = Prompt.ask(
                f"  [bold {BLUE_LIGHT}]Enter new image URL (must be a direct link, e.g., .png/.jpg)[/]")
